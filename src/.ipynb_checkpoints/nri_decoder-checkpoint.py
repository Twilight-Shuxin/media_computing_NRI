...
class MLPDecoder(nn.Module):
    def __init__(self, node_dims, sep_hidden_dims, sep_out_dims, edge_types, hidden_dims, dropout_rate, skip_first=False):
        super().__init__()
        self.layer_sep_1 = nn.ModuleList([nn.Linear(2 * node_dims, sep_hidden_dims) for i in range(edge_types)])
        self.relu1 = nn.ReLU()
        self.drop_out = nn.Dropout(dropout_rate)
        self.layer_sep_2 = nn.ModuleList([nn.Linear(sep_hidden_dims, sep_out_dims) for i in 
                                          range(edge_types)])
        self.relu2 = nn.ReLU()
    ...
    def one_step_ahead(self, node_input, send_mask, rec_mask, encoder_edge_input):
        ...
        fused_input = torch.zeros(edge_rep.size(0), edge_rep.size(1), edge_rep.size(2), self.sep_out_dims)
        ...
        for i in range(start_idx, len(self.layer_sep_1)):
            ...
            processed_node_input = self.relu2(self.layer_sep_2[i](processed_node_input))
            processed_node_input = processed_node_input * encoder_edge_input[:,:,:,i:i+1]
            fused_input += processed_node_input
        # Edge2node
        agg_input = fused_input.transpose(-2, -1).matmul(rec_mask).transpose(-2, -1)
        # Skip connection
